...
public class Dicescript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        manager.value = value; //update the manager
    }

    // Update is called once per frame
    void Update()
    {
        if (value != 0) //if we have a value
        {
        ...
        else if (rb.IsSleeping() && hasLanded && value == 0) //if it has landed, check value
        {
            Value_Check();
            if (value == 0) //if we still have no value, it's stuck
            {
                Unstuck(); 
            }
        }
    ...
    void Value_Check()
    {
        foreach (DiceSides side in sides) //check all sides of the dice
        {
            if (side.OnGround()) 
            {
                ...
                /*if (manager.CurrentPlayerID == 0) //debugging cheat 
                {
                    value = 3;
                }
                else
                ...
                manager.value = value; //our dice roll result
